...
juncPtGraphic::usage="juncPtGraphic[binimg_Image,junctionpts_Image,modbcfilt_,maxrad_,brnchptssize_,junctptsize_,imgsize_,
sklcolor_,JPcolor_]\n Displays a graphic highlighting the skeleton branches and junction points. You can use max rad to average the junction points 
within that radius and show the mean coordinate.";
...
tableDisplay3[filenames_, data_List] := TableForm[Thread[{FileBaseName
   ...
   tableDisplay4[filenames_, data_List,ssdata_List] :=  TableForm[Thread[{FileBaseName
   /@ filenames, Map[If[Length[#]>0, Keys @ Last[#], 0]&, data], Map[Length,ssdata],
   Map[Length@Flatten[#[[All,3]],1]&,ssdata],Map[Total[#[[All,1]]]&,ssdata]}], TableHeadings ->
   {None, {"File names", "# junct pts","# Skl segments",
...
(* ::Input::Initialization:: *)
scaleFunc[data_List,pixsize_,mode_String]:=If[mode==="yes",
...
iv=Map[ImageValue[dt,#]&,Flatten[sscdata[[All,2]],1]];
...
(* ::Input::Initialization:: *)
sklBranchPts[skl_List]:=Block[{selcomp,branchptsimg,endptsimg,count},selcomp=Map[SelectComponents[Binarize[#,0.0001],#Count>2&]&,skl];branchptsimg=Map[MorphologicalTransform[#,"SkeletonBranchPoints",1]&,selcomp];
endptsimg=Map[MorphologicalTransform[#,"SkeletonEndPoints",1]&,selcomp];count=Map[ComponentMeasurements[#,"Count"]&,branchptsimg];{branchptsimg,endptsimg,count}]
...
histoDisplay[data_,fn_,size_]:=
Histogram[data[[All,3]],{3},"Probability",AxesLabel->{"Angle (deg)","Probability"},PlotRange->{{0,55},All},PlotLabel->FileBaseName[fn],ImageSize->size]
...
HighlightImage[trimmedimg,{Green,PointSize[sklptsize],Point[Flatten[branchdat[[All,2]],1]],Red,Thickness[0.001],
Line/@nc,
Blue,PointSize[locptsize],
Point/@pos,
If[circle==="yes",
Thread[Circle[pos,rad]]
]},ImageSize->500]
]
...
scaleBarV3[img_,xfrac_,yfrac_,lenfrac_,imgdim_List,pixsize_,color_,ynumpos_,fontsize_Integer,linethick_,unit_String]:=Block[{len,xmin,xmax,ypos,bkgoffset,graphic1,newimg,newxmin,newxmax,newlen,graphic2},
len=Round[lenfrac*imgdim[[1]]*pixsize,10];
...
newlen=IntegerPart[(newxmax-newxmin)*pixsize];
...
Style[Text[StringJoin[ToString[newlen],unit],{xmin+(xmax-xmin)/2,ypos-ynumpos}],fontsize,Bold,Background->White]}];
...
],RasterSize->(*ImageDimensions[img]*)1500]
]

juncPtGraphic[binimg_Image,junctionpts_Image,modbcfilt_,maxrad_,brnchptssize_,junctptsize_,imgsize_,sklcolor_,JPcolor_]:=Module[{iv,u1,mean},
iv=ImageValuePositions[junctionpts,1];
u1=Union@Map[Sort[#,#1[[1]]<#2[[1]]&]&,Table[Nearest[iv,iv[[i]],{2,maxrad}],{i,Length[iv]}]];
mean=Map[Mean,u1];
HighlightImage[binimg,{PointSize[brnchptssize],sklcolor,modbcfilt[[All,2]],JPcolor,PointSize[junctptsize],Point[mean]},ImageSize->imgsize]
]
